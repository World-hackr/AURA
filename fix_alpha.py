...
def get_absolute_transform(el, parent_map):
    ...
    # multiply matrices (reverse order)
    # Actually, easier to just apply them in order from child to root
    def apply_all(x, y):
        cx, cy = x, y
        for m in matrices:
...
tree = ET.parse(f'backend/parts_library/{comp}/assets/AlphaNumericDisplay-v13_breadboard.svg')
root = tree.getroot()
