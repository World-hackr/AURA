...
def get_absolute_transform(el, parent_map):
    ...
    def apply_all(x, y):
        cx, cy = x, y
        for m in reversed(matrices): # Applying from parent down to child actually! No, the order matters. SVG applies right to left.
            pass
        # Correct approach: to get screen coordinates, you apply child's transform, then parent's, then grandparent's.
        for m in matrices:
...
for pin in d['pins']:
    conn_id = pin['id']
    found_el = None
    for eid in [f"{conn_id}pin", f"{conn_id}pad"]:
        for el in root.iter():
    ...
    if found_el is not None:
        ...
        if tag == 'path':
            ...
            if match:
                ...
                # Add half the size of the square (approx 1.13) to center it
                cx += 1.13
                cy -= 1.13
            else:
                cx, cy = 0, 0
        ...
        print(f"Grove {conn_id} ({pin['label']}): x={cx}, y={cy} -> uX={aura_x}, uY={aura_y}")
        pin['uX'] = aura_x
        pin['uY'] = aura_y
